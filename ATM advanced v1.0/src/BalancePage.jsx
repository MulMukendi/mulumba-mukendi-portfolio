

function BalancePage({user, setPage}) {
  
  //display the balance of the user

  return (
    <div className='card bg-gradient-to-l from-slate-950 to-blue-950'>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="lightgreen" className="h-20 ml-48 ">
        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m3.75 9v7.5m2.25-6.466a9.016 9.016 0 0 0-3.461-.203c-.536.072-.974.478-1.021 1.017a4.559 4.559 0 0 0-.018.402c0 .464.336.844.775.994l2.95 1.012c.44.15.775.53.775.994 0 .136-.006.27-.018.402-.047.539-.485.945-1.021 1.017a9.077 9.077 0 0 1-3.461-.203M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
        </svg>
        <p className='text-4xl mb-6 pt-5 font-medium '>Account Balance</p>
        <div className=' bg-slate-800 rounded-xl w-96 h-36 ml-12 mb-7'>
              <p className='mb-6 pt-3 text-xl text-slate-400'>Available Balance</p>
              <p className='text-6xl font-bold text-green-400'>${user.balance}</p>

        </div>
        <button className="w-96 h-20 rounded-2xl bg-blue-700 mb-5 text-3xl" onClick={()=>{setPage("menuPage")}}>Back to Menu</button>
    </div>
  )
}

export default BalancePage